...
import { ClipboardList, BookOpen, AlertCircle, CheckCircle2 } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { cn } from "@/lib/utils";
import {
  useBorrowRecordStats,
  type BorrowRecordStatsSlice,
...
type StatDef = {
  ...
  countKey: "all" | "borrowed" | "overdue" | "returned";
};

const STAT_DEFS: StatDef[] = [
  {
    slice: "all",
    titleKey: "statsTotalTitle",
    badgeKey: "statsTotalBadge",
    changeType: "neutral",
    icon: ClipboardList,
    color: "blue",
    countKey: "all",
  ...
  {
    slice: "borrowed",
    titleKey: "statsBorrowedTitle",
    badgeKey: "statsBorrowedBadge",
    changeType: "positive",
    icon: BookOpen,
    color: "purple",
    countKey: "borrowed",
  ...
  {
    slice: "overdue",
    titleKey: "statsOverdueTitle",
    badgeKey: "statsOverdueBadgeOk",
    badgeKeyUrgent: "statsOverdueBadgeUrgent",
    changeType: "negative",
    icon: AlertCircle,
    color: "red",
    countKey: "overdue",
  ...
  {
    slice: "returned",
    titleKey: "statsReturnedTitle",
    badgeKey: "statsReturnedBadge",
    changeType: "positive",
    icon: CheckCircle2,
    color: "green",
    countKey: "returned",
...
export function BorrowRecordsStatsCards() {
  const { t } = useTranslation("borrowRecord");
  const { counts, loading } = useBorrowRecordStats();
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {STAT_DEFS.map((stat) => {
        ...
        const isOverdue = stat.slice === "overdue";
        const badgeText =
          isOverdue && raw > 0 && stat.badgeKeyUrgent
            ? t(stat.badgeKeyUrgent)
            : t(stat.badgeKey);
        const effectiveChangeType =
          isOverdue && raw === 0 ? "neutral" : stat.changeType;
